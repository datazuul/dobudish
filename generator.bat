...
rem -----------------------------------------------------------------
rem
rem Driver script for the DOBUDISH docbook environment
rem
rem -----------------------------------------------------------------

setlocal

REM The DOBUDISH root directory
set DOBUDISH_HOME=%~dp0

set SYSTEM_HOME=%DOBUDISH_HOME%\system
...
echo You don't use DobuDish that comes with preinstalled Java.
echo You don't seem to have the variable JAVA_HOME set.
echo To use DobuDish, either download the one that includes a JRE
...
"%JAVA_HOME%\bin\java" -cp "%LOCALCLASSPATH%" %PARAM_LINE% org.apache.tools.ant.Main -buildfile "%DOBUDISH_HOME%\system\etc\build-main.xml" -logger org.apache.tools.ant.NoBannerLogger %ANT_TARGET%
